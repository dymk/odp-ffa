...
pub fn ffa_smc<F: Function>(_: F) -> Result<SmcResult, Error> {
    unimplemented!(
        "ffa_smc is only implemented for aarch64 or non-test builds on other architectures"
    )
}
...
mod test_ffa_smc_impl {
    ...
    thread_local! {
        static SMC_CALL_COUNT_INTERNAL: Cell<Vec<SmcCall>> = Cell::new(Vec::new());
    }
...
mod tests {
    ...
    fn test_smc_params_from_iter() {
        let smc_params = match SmcParams::try_from_iter((1..18).into_iter()) {
            Ok(params) => params,
        ...
        assert_eq!(
            SmcParams::try_from_iter((1..19).into_iter()),
            Err(Error::TooManySmcParams)
        );
    }
}
