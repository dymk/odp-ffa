use crate::{
    exec_simple, util::combine_low_high_u32, Error, ExecResult, Function, FunctionId, SmcParams,
};
...
impl Function for NotificationGet {
    ...
    fn exec(self) -> ExecResult<Self::ReturnType> {
        exec_simple(self, |result| {
            Ok(Response {
                sp_notifications_bitmap: combine_low_high_u32(
                    result.params.x2 as u32,
                    result.params.x3 as u32,
                ),
                vm_notifications_bitmap: combine_low_high_u32(
                    result.params.x4 as u32,
                    result.params.x5 as u32,
                ),
                fw_notifications_bitmap: combine_low_high_u32(
                    result.params.x6 as u32,
                    result.params.x7 as u32,
                ),
            })
        })
    }
...
mod tests {
    ...
    fn test_notification_get_round_trip(
        #[case] cpu_id: u16,
        #[case] endpoint_id: u16,
        #[case] flags: u32,
    ) {
