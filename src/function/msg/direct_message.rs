...
use crate::{
    smc::SmcParams, util::combine_low_high_u16, Error, HasRegisterPayload, Payload, RegisterPayload,
};
...
impl TryFrom<SmcParams> for DirectMessage {
    ...
    fn try_from(value: SmcParams) -> Result<Self, Self::Error> {
        ...
        let payload_regs = [
            value.x4, value.x5, value.x6, value.x7, value.x8, value.x9, value.x10, value.x11,
            value.x12, value.x13, value.x14, value.x15, value.x16, value.x17,
        ];
        let payload_bytes_iter = payload_regs
            .iter()
            .flat_map(|&reg| u64::to_le_bytes(reg).into_iter());
