...
impl TryInto<SmcParams> for Console<'_> {
    ...
    fn try_into(self) -> Result<SmcParams, Self::Error> {
        ...
        let string_len = self.bytes.len() as u64;
        SmcParams::try_from_iter(once(string_len).chain(char_data_iter))
    }
}
...
mod tests {
    ...
    fn test_console_writer() {
        ...
        for (id, (expected, actual)) in parts.iter().zip(get_smc_calls().iter()).enumerate() {
            ...
            assert_eq!(
                ...
                actual.params.x1,
                "[{id}]: expected: {:?}, actual: {:?}",
                core::str::from_utf8(&expected_bytes).unwrap_or(""),
                core::str::from_utf8(&u64::to_le_bytes(actual.params.x2)).unwrap_or("")
            );
